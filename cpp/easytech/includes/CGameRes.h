...
struct CGameRes {
    ...
    std::map(std::string, ecImage*) ImageCountryMedal;
    ...
    std::map(std::string, ecImage*) Image_flag;
    ...
    struct ecImage *Image_buildmark_oilwell;
    std::map<std::string,std::vector<ecImage*>> map_countryArmyImages;
    struct ecImage* Image_OrganizationFill; //注意，需要初始化
    struct ecImage* Image_newCardmark[10] ;
    struct ecImage* Image_circle;//注意，需要初始化
#ifdef __cplusplus
    //绘制番号
    ...
    ecImage *GetBaseImage(int armyCount,int allance);
